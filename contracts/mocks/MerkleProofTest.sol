...
contract MerkleProofTest{
    ...
    function verifyMultiple(bytes32[][] memory proofs, bytes32 root, bytes32 [] memory leaves, uint32 [] memory assetId, uint32 depth) external view returns (bool) {
        return MerkleProof.verifyMultiple(proofs, root, leaves, assetId, depth);
    }
